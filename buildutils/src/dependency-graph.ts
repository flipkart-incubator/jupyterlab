...
function main({
  ...
  if (!jupyterlab) {
    Object.keys(sub).forEach(v => {
      sub[v] = sub[v].filter(w => !w.startsWith('@fk-jupyterlab/'));
    });
    Object.keys(sub).forEach(v => {
      if (v.startsWith('@fk-jupyterlab/') && sub[v].length === 0) {
        delete sub[v];
      }
    });
  }
...
commander
  ...
  .option(
    '--no-jupyterlab',
    'Do not include dependency connections TO @fk-jupyterlab org packages nor isolated @fk-jupyterlab org packages'
  )
