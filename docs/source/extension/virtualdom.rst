...
widgets using the ``ReactWidget`` class from ``@fk-jupyterlab/apputils``:
...
To have your React element change in response to a signal event, use the ``UseSignal`` component from ``@fk-jupyterlab/apputils``,
